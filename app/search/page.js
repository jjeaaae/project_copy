...
export default function SearchPage() {
  ...
  }, [nameQuery, cuisine, difficulty, maxTime, maxCalories, ageGroup]);
  ...
  // ingredient include/exclude is filtered client-side
  const results = allResults
    ...
    .filter((r) =>
      excluded.every(
        (ing) => !r.ingredients.some((i) => i.toLowerCase().includes(ing.toLowerCase()))
      )
    );
        ...
        <div className="container hero-top">
          <div className="hero-copy">
            ...
            <p className="section-intro">
              ...
              гарал үүсэл зэргээр эмхтэй хайж өнөөдрийн жороо сонгоорой.
            </p>
            {included.length > 0 && (
              <div className="hero-notes">
                {included.map((ing) => <span key={ing}>{ing}</span>)}
              </div>
            )}
          </div>
              ...
              <div className="token-row">
                {included.map((ing) => (
                  <span key={ing} className="token">
                    {ing}
                    <button
                      onClick={() => setIncluded(included.filter((i) => i !== ing))}
                ...
                <input
                  ...
                  onKeyDown={(e) => e.key === "Enter" && addIncluded(includeInput)}
                />
              </div>
            </div>
              ...
              <div className="token-row">
                {excluded.map((ing) => (
                  <span key={ing} className="token token-danger">
                    {ing}
                    <button
                      onClick={() => setExcluded(excluded.filter((i) => i !== ing))}
                      style={{ marginLeft: 6, background: "none", border: "none", cursor: "pointer", fontWeight: 700 }}
                    >×</button>
                  </span>
                ))}
                <input
                  ...
                  onKeyDown={(e) => e.key === "Enter" && addExcluded(excludeInput)}
                />
              </div>
            </div>
            ...
            <div className="quick-row">
              <div className="quick-add">
                ...
                {QUICK_CHIPS.map((chip) => (
                  <button key={chip} type="button" className="quick-chip" onClick={() => addIncluded(chip)}>
                    {chip}
                  </button>
                ))}
              </div>
          ...
          <div className="search-layout">
            <aside className="filters-panel panel" aria-label="Шүүлтүүд">
              ...
              <section className="filter-section">
                ...
                <select className="field-control" value={cuisine} onChange={(e) => setCuisine(e.target.value)}>
              ...
              <section className="filter-section">
                <h3>Насны ангилал</h3>
                <select className="field-control" value={ageGroup} onChange={(e) => setAgeGroup(e.target.value)}>
                  <option value="">Бүгд</option>
                  {AGE_GROUPS.map((a) => (
            ...
            <section className="results-area" aria-label="Хайлтын үр дүн">
              ...
              <div className="result-stack">
                {results.map((recipe) => (
                  <article key={recipe._id} className="match-card">
                    <img className="match-image" src={recipe.image} alt={recipe.alt} />
                      ...
                      <div className="match-tags" style={{ marginTop: 16 }}>
                        {recipe.ingredients.map((ing) => (
                          <span
                            key={ing}
                            className={
                              included.some((i) => ing.toLowerCase().includes(i.toLowerCase()))
                                ? "ok-tag"
                                : "soft-tag"
                            }
                          >
                    ...
                    <div className="match-score good">
                      ...
                      <a
                        ...
                        style={{ marginTop: 8, textAlign: "center", textDecoration: "none", padding: "10px 16px", borderRadius: 14 }}
                      >
                        Жор үзэх
                      </a>
                    </div>
